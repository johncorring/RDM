function [energy, grad] = rdmL2Fixed_Matrix(param, init_affine, basis, kernel, ... 
    Tx, Ty, Tz, Vtarget, target, Vtemplate, MatX, MatY, scale, lambda, ...
    alpha, beta, n, d)
...
if( d == 2)
    for i = 1:size(T1,1)
           ...
           %[U1 S1 V1] = svd(Ai);
            ...
            %MatX(2*(i-1)+1:2*(i-1)+2,:) = Rot' * diag([1.2/scale^2; .8*ones(d-1,1)/scale^2]) * Rot;
...
elseif( d == 3)
    for i = 1:size(T1,1)
        ...
        %MatX(2*(i-1)+1:2*(i-1)+2,:) = Rot' * diag([1.2; .8*ones(d-1,1)]) * Rot;
        MatX(d*(i-1)+1:d*(i-1)+d,:) = Rotb' * diag([1.2/scale^2; .8*ones(d-1,1)/scale^2]) * Rotb;
