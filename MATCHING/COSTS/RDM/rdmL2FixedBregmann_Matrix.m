function [energy, grad] = rdmL2FixedBregmann_Matrix(param, oldP, init_affine, basis, kernel, ... 
    Tx, Ty, Tz, Vtarget, target, Vtemplate, MatX, MatY, ...
    scale, lambda, alpha, beta, n, d)
...
bending = trace((tps_param - oldP)'*kernel*(tps_param - oldP));
...
m = size(T1,1);
%{
MatX = zeros(d*size(T1,1), d);
MatY = zeros(d*n1, d);

for b = 1:n1
    %MatY(d*(b-1)+1:d*b,:) = eye(d)/scale^2;
    
        [Rotb rb] = qr([Vtarget(d*(b-1)+1:d*b)'; eye(d)]');

        %MatX(2*(i-1)+1:2*(i-1)+2,:) = Rot' * diag([1.2; .8*ones(d-1,1)]) * Rot;
        MatY(d*(b-1)+1:d*(b-1)+d,:) = Rotb' * diag([1.2/scale^2; .8*ones(d-1,1)/scale^2]) * Rotb;
end
%}
if( d == 2)
    for i = 1:size(T1,1)
           ...
           %[U1 S1 V1] = svd(Ai);
            ...
            %Rot = qr([Vd(i,:); eye(d)]');%[Vd(i,:)/norm(Vd(i,:)); [-Vd(i,2) Vd(i,1)]/norm(Vd(i,:))];
            %Rot = Rot(:,2:end);
            MatX(2*(i-1)+1:2*(i-1)+2,:) = Ai * MatX(2*(i-1)+1:2*(i-1)+2,:) * Ai';
            %Rot' * diag([1.2/scale^2; .8*ones(d-1,1)/scale^2]) * Rot;
...
elseif( d == 3)
    for i = 1:size(T1,1)
        ...
        Ai = [T1(i,:); T2(i,:); T3(i,:)]';
        Vd(i,:) = (Ai*Vtemplate(d*(i-1)+1:d*i));
        [Rotb rb] = qr([Vd(i,:); eye(d)]');
        %MatX(2*(i-1)+1:2*(i-1)+2,:) = Rot' * diag([1.2; .8*ones(d-1,1)]) * Rot;
        MatX(d*(i-1)+1:d*(i-1)+d,:) = Ai' * MatX(d*(i-1)+1:d*(i-1)+d,:) * Ai;
        %Rotb' * diag([1.2/scale^2; .8*ones(d-1,1)/scale^2]) * Rotb;
...
%}
grad(d+2:n,:) = grad(d+2:n,:) + 2*beta*kernel*(tps_param - oldP);
