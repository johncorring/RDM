function [energy, grad] = gmmreg_L2_tps_costfunc(param, init_affine, basis, kernel, Tx, Ty, Tz, Vmodel, model, Vscene, scale, lambda, alpha, beta, n, d)

kappa1 = 1/scale;
[n1] = length(Vmodel);
stN = d*size(Tx,2);
if isempty(init_affine)
    flips_param = param(end-2*n1/d+1:end-n1/d);
...
%Vd = Vd./repmat(sqrt(sum(Vd.^2,2)),1,2);
if(d == 2)
    vnew = Vmodel.*kron(flips_param',[1; 1]);
elseif (d == 3)
    vnew = Vmodel.*kron(flips_param',[1; 1; 1]);
end

[energy, gradm1, gradv1, gradpa1] = ... 
    gab_costfunct(model, after_tps, vnew, Vlong, coeff_param, ones(m,1)/m,  scale, lambda);
[energy, gradm2, gradv2, gradpa2] = ... 
    gab_costfunct(after_tps, model, Vlong, vnew, ones(m,1)/m, coeff_param, scale, lambda);
