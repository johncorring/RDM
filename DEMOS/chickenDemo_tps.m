...
v0 = cos(kron(v1,ones(2,1)) - kron(ones(n1,1), pi/2*[0; 1]));
v1 = cos(kron(v1,ones(2,1)) - kron(ones(n1,1), pi/2*[0; 1]));


%load a template chicken
x2 = ckshapes{10}(5:15:end,:);
...
config = rdmConfig(x1, x2, v1', v2', 'tps', 'fixed', [''], sigma, lambda, 0);
...
for i = 1:3
    ...
    if(1)
        ...
        pk = history.x(end,:);
        %transform grid
        [twarp] = transform_pointset(twarp, 'tps-free', ... 
            [pk], config.ctrl_pts);
        ...
        hold on; quiver(x2p(:,1), x2p(:,2), v2p(1:2:end), v2p(2:2:end), '-bx', 'LineWidth', 3);
